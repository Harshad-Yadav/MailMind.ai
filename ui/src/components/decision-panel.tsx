  ...
  return (
    <Card className="command-surface animate-rise overflow-hidden border-white/70 bg-white/75 shadow-panel backdrop-blur" style={{ animationDelay: "120ms" }}>
      ...
      <CardContent className="space-y-6 p-6">
        ...
        <div className="flex flex-col gap-4 rounded-[1.6rem] border border-slate-200/60 bg-white/40 p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            ...
            <div className="text-xs text-slate-500">Switch to AUTO MODE to let the agent suggest actions.</div>
          </div>
          <div className="flex items-center gap-1 rounded-xl border border-slate-200 bg-slate-100/50 p-1 shadow-inner">
            <button 
              onClick={() => setAutoMode(false)} 
              className={`flex items-center gap-2 rounded-lg px-4 py-2 text-xs font-bold uppercase transition-all ${
                !autoMode ? 'bg-steel text-white shadow-md' : 'text-slate-500 hover:bg-slate-200/50'
              }`}
            >
              Manual
            </button>
            <button 
              ...
              className={`flex items-center gap-2 rounded-lg px-4 py-2 text-xs font-bold uppercase transition-all ${
                autoMode ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-200/50'
              }`}
            >
              Auto Mode
              <Sparkles className={`h-3 w-3 ${autoMode ? 'animate-pulse text-emerald-200' : 'text-slate-400'}`} />
            </button>
          </div>
        </div>
